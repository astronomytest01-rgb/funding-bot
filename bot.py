...
def check_recent_trend(fetcher, coin, direction, n=6):
    """
    Проверка последних n (6) ставок на разворот тренда.
    Минимум половина ставок должна быть 'в нашу пользу', 
    и средняя за это время не должна быть пробита резким сквизом в обратную сторону.
    """
    try:
        now_ms   = int(time.time() * 1000)
        start_ms = now_ms - 3 * 24 * 60 * 60 * 1000 # берем запас в 3 дня
...
async def ai_got_coin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not coins:
        await update.message.reply_text("Не распознал монеты. Попробуй: `ENJ BTC`")
        return WAIT_AI_COIN
    await do_ai_multiple(update, coins)
    return ConversationHandler.END
...
async def do_ai_multiple(update, coins):
    ...
    await msg.reply_text(f"🤖 Начинаю детальный анализ {len(coins)} монет(ы)...")
...
async def an_run_scan(trigger, context: ContextTypes.DEFAULT_TYPE):
    ...
    # BULK АНАЛИЗ ЧЕРЕЗ GEMINI С ДРОБЛЕНИЕМ ПО 15 МОНЕТ
    ...
    await msg.reply_text(
        f"🤖 *Gemini фильтрует список из {len(target_coins)} монет (порциями по 15)...*",
        parse_mode="Markdown"
    )
    ...
    for i in range(0, len(target_coins), CHUNK_SIZE):
        chunk = target_coins[i:i+CHUNK_SIZE]
        coins_list_str = "\n".join([f"- {c} (Направление: {'ЛОНГ' if d == 'LONG' else 'ШОРТ'})" for c, d, a in chunk])
        ans = gemini_analyze_bulk(coins_list_str, days)
        if ans and "Подходящих фундаментальных монет нет" not in ans:
            bulk_results.append(ans)
        time.sleep(3)
    ...
    if bulk_results:
        ...
        await msg.reply_text("🤖 Gemini не дал рекомендаций или забраковал все монеты.")
...
async def auto_scan_job(context):
    ...
    await context.bot.send_message(
        chat_id=chat_id,
        text=f"🤖 *Gemini фильтрует {len(unique_coins)} монет(ы) (порциями по 15)...*",
        parse_mode="Markdown"
    )

    # ДРОБИМ СПИСОК ПО 15 ШТУК
    unique_coins_list = list(unique_coins.items())
    CHUNK_SIZE = 15
    bulk_results = []
    
    for i in range(0, len(unique_coins_list), CHUNK_SIZE):
        chunk = unique_coins_list[i:i+CHUNK_SIZE]
        coins_list_str = "\n".join([f"- {c} (Направление: {'ЛОНГ' if d == 'LONG' else 'ШОРТ'})" for c, d in chunk])
        ans = gemini_analyze_bulk(coins_list_str, AUTO_SCAN_DAYS)
        if ans and "Подходящих фундаментальных монет нет" not in ans:
            bulk_results.append(ans)
        time.sleep(3)
        
    bulk_analysis = "\n".join(bulk_results) if bulk_results else ""
