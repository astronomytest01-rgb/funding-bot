...
REPORT_CHAT_ID  = os.environ.get("REPORT_CHAT_ID", "")  # куда слать авто-отчёт (твой chat_id)
...
# ─────────────────────────────────────────────
# ПРОВЕРКА АКТУАЛЬНОСТИ ТРЕНДА (последние 24ч)
# ─────────────────────────────────────────────

def check_recent_trend(fetcher, coin, direction, n=6):
    """Берёт последние n ставок (покрывает 24ч при выплатах каждые 4/8ч).
    Возвращает True если тренд актуален, False если монета уже переломилась.
    
    Для LONG: последние ставки должны оставаться отрицательными (avg < 0).
    Для SHORT: последние ставки должны оставаться положительными (avg > 0).
    """
    try:
        now_ms   = int(time.time() * 1000)
        start_ms = now_ms - 2 * 24 * 60 * 60 * 1000  # берём 2 дня чтобы точно получить 6 записей
        rows, _ = fetcher(coin, start_ms, now_ms)
        if not rows:
            return True  # нет данных — не отсекаем, пусть проходит
        # сортируем по времени от новых к старым и берём последние n
        ...
        if direction == "LONG":
            return avg_recent < -0.005  # среднее последних ставок должно быть отрицательным
        else:
            return avg_recent > 0.005   # среднее последних ставок должно быть положительным
    except Exception:
...
GEMINI_PROMPT_TEMPLATE = """Монета {coin}. Направление фандинга: {direction}, ставка {avg_rate:+.4f}% за {days} дней.

Напиши ровно 3 абзаца в указанном порядке. Только обычный текст — никаких звёздочек, решёток, подчёркиваний. Каждый абзац начинается с эмодзи и заканчивается точкой.

✅ Рекомендация: брать или нет в дельта-нейтральную позицию прямо сейчас, оптимальное плечо x1/x2/x3/x4 и почему именно оно. 2-3 завершённых предложения.

📊 Волатильность: типичный дневной и недельный диапазон движения {coin}, насколько опасны резкие иглы при указанном плече, стоит ли держать стоп. 2-3 завершённых предложения.

📌 Проект: чем занимается {coin}, сегмент (DeFi/L1/RWA/гейминг и т.д.), есть ли ближайшие события которые могут сломать арбитраж. 1-2 завершённых предложения.

Не обрывай текст. Все предложения должны быть завершены."""


def gemini_analyze_coin(coin, direction, avg_rate, exchange, days):
    """Запрашивает анализ монеты у Gemini. Возвращает текст или None при ошибке.
    При rate limit (429) делает до 3 попыток с паузой."""
...
async def an_run_scan(trigger, context: ContextTypes.DEFAULT_TYPE):
    ...
    for batch_idx, batch in enumerate(batches):
        ...
        for coin in batch:
            ...
            if method == "rate":
                ...
                category  = r["category"]
                # Проверяем актуальность тренда по последним 6 ставкам (24ч)
                if not check_recent_trend(fetcher, coin, direction):
                    time.sleep(0.15)
                    continue
                ...
                if neg_ratio >= MIN_NEG_RATIO and avg_rate < 0:
                    ...
                    if daily_income >= threshold:
                        if check_recent_trend(fetcher, coin, "LONG"):
                ...
                if pos_ratio >= MIN_POS_RATIO and avg_rate > 0:
                    ...
                    daily_income     = amount * abs(avg_rate) / 100 * payments_per_day
                    if daily_income >= threshold:
                        if check_recent_trend(fetcher, coin, "SHORT"):
        ...
        # Тихий прогресс — одно сообщение раз в ~25% прогресса
    ...
    # ── Gemini анализ для монет которые точно проходят (full) ────────────
    if not GEMINI_API_KEY:
        return

    if method == "income":
        full_coins = [(c, d, a) for c, a, o, d, cat, i in passed]
    else:
        full_coins = [(c, d, a) for c, a, o, d, cat, _ in passed if cat == "full"]

    if not full_coins:
        return

    await msg.reply_text(
        f"🤖 *Gemini анализирует {len(full_coins)} монет...* Это займёт около минуты.",
        parse_mode="Markdown"
    )

    for coin, direction, avg_rate in full_coins:
        analysis = gemini_analyze_coin(coin, direction, avg_rate, exchange, days)
        if analysis:
            dir_icon = "🟢 ЛОНГ" if direction == "LONG" else "🔴 ШОРТ"
            # Заголовок с Markdown, текст Gemini — без parse_mode чтобы не обрезало
            await msg.reply_text(
                f"🤖 *{coin}* | {dir_icon}",
                parse_mode="Markdown"
            )
            # Шлём текст Gemini без Markdown — безопасно от обрезания
            if len(analysis) > 4000:
                for chunk in [analysis[i:i+4000] for i in range(0, len(analysis), 4000)]:
                    await msg.reply_text(chunk)
            else:
                await msg.reply_text(analysis)
        time.sleep(10)  # пауза между запросами к Gemini
...
# ─────────────────────────────────────────────
# АВТО-СКАН (ежедневно в 20:00)
# ─────────────────────────────────────────────

AUTO_SCAN_AMOUNT    = 20000   # размер позиции $
AUTO_SCAN_THRESHOLD = 29      # минимальный доход $/день
AUTO_SCAN_DAYS      = 3       # период анализа в днях
AUTO_SCAN_EXCHANGES = ["phemex", "xt", "toobit", "coinw", "okx", "bingx"]

async def auto_scan_job(context):
    """Ежедневный авто-скан по всем биржам в 20:00."""
    ...
    await context.bot.send_message(
        ...
        text=f"🕗 *Авто-скан* {now_str}\n"
             f"Биржи: {', '.join(AUTO_SCAN_EXCHANGES)}\n"
             f"Метод: доходность ${AUTO_SCAN_AMOUNT:,} ≥${AUTO_SCAN_THRESHOLD}/день за {AUTO_SCAN_DAYS} дней",
        parse_mode="Markdown"
    )

    all_passed = []  # (coin, avg_rate, outlier, direction, exchange_label)

    for exchange in AUTO_SCAN_EXCHANGES:
        if not EXCHANGES_ENABLED.get(exchange):
            continue
        ...
        for coin in coins:
            ...
            pos_ratio = len(pos) / len(clean)
            avg_rate  = sum(clean) / len(clean)
            payments_per_day = len(clean) / AUTO_SCAN_DAYS

            # ЛОНГ
            if neg_ratio >= MIN_NEG_RATIO and avg_rate < 0:
                daily_income = AUTO_SCAN_AMOUNT * abs(avg_rate) / 100 * payments_per_day
                if daily_income >= AUTO_SCAN_THRESHOLD:
                    if check_recent_trend(fetcher, coin, "LONG"):
                        below   = sum(1 for r in clean if r <= STABILITY_THRESHOLD)
                        outlier = (len(clean) - below) / len(clean) * 100
                        passed_exchange.append((coin, avg_rate, outlier, "LONG", label))
                        all_passed.append((coin, avg_rate, outlier, "LONG", label))
                time.sleep(0.1)
                continue
            ...
            if pos_ratio >= MIN_POS_RATIO and avg_rate > 0:
                ...
                if daily_income >= AUTO_SCAN_THRESHOLD:
                    if check_recent_trend(fetcher, coin, "SHORT"):
                        above   = sum(1 for r in clean if r >= -STABILITY_THRESHOLD)
                        outlier = (len(clean) - above) / len(clean) * 100
                        passed_exchange.append((coin, avg_rate, outlier, "SHORT", label))
                        all_passed.append((coin, avg_rate, outlier, "SHORT", label))
                time.sleep(0.1)
                continue
        ...
        # Отчёт по бирже
        if passed_exchange:
            longs  = [(c,a,o) for c,a,o,d,_ in passed_exchange if d == "LONG"]
            shorts = [(c,a,o) for c,a,o,d,_ in passed_exchange if d == "SHORT"]
            lines  = [f"📊 *{label}* — найдено {len(passed_exchange)}:"]
            if longs:
                lines.append(f"\n🟢 *ЛОНГ* ({len(longs)}):")
                for c,a,o in sorted(longs, key=lambda x: x[1]):
                    lines.append(f"  `{c}` avg `{a:+.4f}%` выбр `{o:.0f}%`")
            if shorts:
                lines.append(f"\n🔴 *ШОРТ* ({len(shorts)}):")
                for c,a,o in sorted(shorts, key=lambda x: -x[1]):
                    lines.append(f"  `{c}` avg `{a:+.4f}%` выбр `{o:.0f}%`")
            await context.bot.send_message(
                chat_id=chat_id,
                text="\n".join(lines),
            ...
            await context.bot.send_message(
                chat_id=chat_id,
                text=f"📊 *{label}* — ничего не найдено",
                parse_mode="Markdown"
            )
    ...
    # Gemini анализ для всех найденных монет
    if not GEMINI_API_KEY or not all_passed:
        return
...
def main():
    ...
    # Авто-скан в 20:00 UTC ежедневно
    if REPORT_CHAT_ID and app.job_queue:
        ...
        print("✅ Авто-скан запланирован на 20:00 UTC")
