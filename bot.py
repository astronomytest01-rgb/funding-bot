...
✅ [МОНЕТА] — [Буквально 3-5 слов: почему подходит, например: Надежный L1, умеренная волатильность]
...
# ─────────────────────────────────────────────
# /ai — ДЕТАЛЬНЫЙ АНАЛИЗ ОДНОЙ МОНЕТЫ
# ─────────────────────────────────────────────

async def cmd_ai_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.args:
        coin = context.args[0].upper()
        await do_ai_single(update, coin)
        return ConversationHandler.END
    await update.message.reply_text("Введите тикер монеты для детального AI-анализа (например: ENJ):")
    return WAIT_AI_COIN
...
async def do_ai_single(update, coin):
    msg = update.message if hasattr(update, 'message') else update
    await msg.reply_text(f"🤖 Собираю данные Gemini AI по монете *{coin}*...", parse_mode="Markdown")
    analysis = gemini_analyze_single(coin)
    if not analysis:
        await msg.reply_text("❌ Ошибка при запросе к Gemini API. Проверь ключи или лимиты.")
        return
...
# ─────────────────────────────────────────────
# РУЧНОЙ СКАН (/analyze и /analyze_ai)
# ─────────────────────────────────────────────
...
async def an_run_scan(trigger, context: ContextTypes.DEFAULT_TYPE):
    ...
    await msg.reply_text(
        ...
        f"Период: *{days} дней*\n"
        f"Режим AI: *{'Включен (массовый фильтр)' if use_ai else 'Выключен'}*\n",
        parse_mode="Markdown"
    )
...
# ─────────────────────────────────────────────
# АВТО-СКАН (ЕЖЕДНЕВНО В 20:00 ПО КИЕВУ)
# ─────────────────────────────────────────────
...
async def auto_scan_job(context):
    ...
    await context.bot.send_message(
        chat_id=chat_id,
        text=f"🤖 *Gemini анализирует список из {len(unique_coins)} монет...*",
        parse_mode="Markdown"
    )
    ...
    if bulk_analysis:
        await context.bot.send_message(chat_id=chat_id, text=f"🤖 *GEMINI AI ОДОБРЯЕТ:*\n\n{bulk_analysis}")
    else:
        await context.bot.send_message(chat_id=chat_id, text="🤖 Подходящих фундаментальных монет не найдено.")
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "/analyze_ai — скан с массовым AI-фильтром (список)\n"
        "/ai — детальный разбор одной монеты через Gemini\n"
...
async def cmd_analyze_start_ai(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("🤖 Скан с Gemini AI (Список)\n\nШаг 1/3: Выбери биржу:", reply_markup=keyboard)
    return AN_METHOD
