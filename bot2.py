...
GEMINI_PROMPT_TEMPLATE = """Ты — мой крипто-аналитик. Дай анализ монеты {coin} для стратегии дельта-нейтрального арбитража фандинга с плечом x3–x4.

Контекст из сканера фандинга:
- Направление: {direction}
- Средняя ставка фандинга: {avg_rate:+.4f}% за период
- Биржа: {exchange}
- Период анализа: {days} дней

Дай ровно такой формат ответа:

Монета {coin} — вводка в 2–3 предложения
Коротко, зачем этот токен существует и почему он может быть интересен для стратегии фандинга.

1. Фундаментальный анализ
– Статус проекта сейчас (что делает, какой сегмент: DeFi, RWA, L1, гейминг и т.д.)
– Активность команды/разработчиков (суть без ссылок)
– Ключевые катализаторы (апдейты, хардфорки, сжигания, листинги, партнёрства)
– Основные риски (регуляторные, конкуренты, токеномика, централизация)

2. Ликвидность и волатильность
– Ликвидность: на каких топ-биржах есть, глубина стакана, делисты/ограничения
– Волатильность: диапазон колебаний и риски для плеча x3–x4
– Насколько опасны «иглы» для ликвидации при плече

3. Фандинг и арбитраж
– Как ведёт себя фандинг обычно: чаще положительный, нулевой или отрицательный
– Есть ли структурное преимущество для дельта-нейтральной стратегии
– Важные моменты календаря (анлоки, голосования, ивенты), которые могут ломать арбитраж

4. Вердикт для моей стратегии
Пригодность для x3: [оценка /10] + короткий комментарий
Пригодность для x4: [оценка /10] + короткий комментарий
Надёжность долгосрок: [низкая/средняя/высокая] + почему

Короткая рекомендация по управлению риском:
– стоит ли брать эту монету для арбитража фандинга
– если да — основные ограничения по плечу и вниманию к новостям/событиям

Пиши живым понятным языком. Не объясняй методологию, сразу давай анализ."""
