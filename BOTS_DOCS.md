...
- зарегистрирован прямым `CommandHandler` до `unknown`; пошаговый режим хранит ожидание монеты/периода в `user_data`, чтобы команда не проваливалась в "Не знаю такой команды" после Railway deploy/restart.
...
- биржа -> метод -> период;
- long-running scan uses async sleeps and `asyncio.to_thread` for network/OI calls, so Telegram polling is not blocked during large exchange scans;
...
6. Во время длинного `/analyze` бот продолжает отвечать на Telegram-команды; сетевые fetch/OI вызовы не блокируют event loop.
...
11. `/oi` проходит сценарий монета -> биржа -> рекомендация и использует те же OI-пороги, что фильтры.
12. Вечерний отчёт использует только `full`-монеты, скрывает OI ниже $500k, показывает OI-статус long/short бирж, Gemini показывает как рекомендацию, считает прибыль за 1 день и отправляет инструкцию входа.
13. CoinW и Bitunix временно отключены: они не появляются в кнопках и не работают через прямые команды, но API-код сохранён для быстрого возврата.
14. Оба `.md` файла обновлены перед push.
