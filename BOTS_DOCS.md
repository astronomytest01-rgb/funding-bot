...
10. `/oi` проходит сценарий монета -> биржа -> рекомендация и использует те же OI-пороги, что фильтры.
11. Вечерний отчёт использует только `full`-монеты, скрывает OI ниже $500k, показывает OI-статус long/short бирж, Gemini показывает как рекомендацию, считает прибыль за 1 день и отправляет инструкцию входа.
12. CoinW и Bitunix временно отключены: они не появляются в кнопках и не работают через прямые команды, но API-код сохранён для быстрого возврата.
13. Оба `.md` файла обновлены перед push.
