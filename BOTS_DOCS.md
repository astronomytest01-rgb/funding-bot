...
8. Вечерний отчёт использует только `full`-монеты, скрывает OI ниже $500k, показывает OI-статус long/short бирж, Gemini показывает как рекомендацию, считает прибыль за 1 день и отправляет инструкцию входа.
