...
- `/funding` — история ставок. Зарегистрирована прямым `CommandHandler` до `unknown`; быстрый ввод `/funding ENJ phemex 7` и пошаговый режим `/funding` -> `ENJ` используют `user_data` fallback.
...
10. OI ниже $500k скрывается в `/analyze` и вечернем отчёте; $500k-$1m показывает ⚠️, $1m+ показывает ✅.
11. `/oi` вручную проверяет OI: монета -> биржа -> рекомендация заходить или пропустить.
12. Вечерний отчёт запускается в 20:00 Europe/Kyiv при наличии `REPORT_CHAT_ID`; `/report` запускает тот же отчёт вручную.
13. CoinW и Bitunix временно отключены: скрыты из кнопок и не работают через прямые команды, API-код оставлен для быстрого возврата.
14. Документация обновлена перед push.
