...
- `/longfunding`: долгосрочный funding-скан -> summary -> первая порция монет -> кнопка `Показать ещё`; OI/24h volume показываются как warning, но не скрывают связки.
...
16. Кнопка `Показать ещё` отдаёт следующую порцию сохранённого результата без повторного скана.
17. Longterm auto job запускается в 21:00 Europe/Kyiv при наличии `REPORT_CHAT_ID`.
18. CoinW и Bitunix временно отключены: скрыты из кнопок и не работают через прямые команды, API-код оставлен для быстрого возврата.
19. Документация обновлена перед push.
