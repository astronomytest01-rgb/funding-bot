...
- `/longfunding`: долгосрочный funding-скан -> summary -> отдельные сообщения по монетам; OI/24h volume показываются как warning, но не скрывают связки.
...
16. Longterm auto job запускается в 21:00 Europe/Kyiv при наличии `REPORT_CHAT_ID`.
17. CoinW и Bitunix временно отключены: скрыты из кнопок и не работают через прямые команды, API-код оставлен для быстрого возврата.
18. Документация обновлена перед push.
