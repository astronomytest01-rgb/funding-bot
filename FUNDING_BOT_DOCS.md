...
- `/oi` — пошаговая проверка Open Interest монеты по одной или нескольким выбранным биржам.
...
- `/oi`: монета -> мультивыбор бирж кнопками -> подтверждение -> отдельная рекомендация OI по каждой выбранной бирже: 🚫 ниже $500k, ⚠️ $500k-$1m, ✅ $1m+.
...
12. OI ниже $500k скрывается в `/analyze` и вечернем отчёте; $500k-$1m показывает ⚠️, $1m+ показывает ✅.
13. `/oi` вручную проверяет OI: монета -> мультивыбор бирж -> подтверждение -> рекомендация заходить или пропустить по каждой выбранной бирже.
14. Вечерний отчёт запускается в 20:00 Europe/Kyiv при наличии `REPORT_CHAT_ID`; `/report` запускает тот же отчёт вручную.
15. CoinW и Bitunix временно отключены: скрыты из кнопок и не работают через прямые команды, API-код оставлен для быстрого возврата.
16. Документация обновлена перед push.
