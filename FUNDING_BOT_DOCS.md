...
- `/longfunding`: ручной долгосрочный funding-скан без Gemini -> summary -> первая порция монет -> кнопка `Показать ещё`; OI/24h volume показываются как warning, но не скрывают связки.
...
Берёт только `full`-монеты, применяет фильтр последних 4 ставок, скрывает подтверждённый OI ниже `$500k` и подтверждённый 24h volume ниже `$400k`, затем ищет лучшую дельта-нейтральную пару по дневной доходности. Funding нормализуется по числу выплат в день: 1h/4h/8h биржи сравниваются как дневный funding, а не как одна ставка. Live-спред входа считается по top-of-book формуле `(SHORT bid / LONG ask * 100) - 100`; маршруты со spread ниже `AUTO_REPORT_MIN_ENTRY_SPREAD_PCT`, по умолчанию `-0.8%`, не показываются, а бот пробует следующий маршрут по этой монете. Маршруты с оценкой ниже `AUTO_REPORT_MIN_DAILY_USD`, по умолчанию `$10/день`, и отрицательные маршруты не показываются. В отчёте выводится OI/Vol24h-статус long/short бирж, число выплат/день, чистый funding за день, оценка прибыли за 1 день на позицию `AUTO_SCAN_AMOUNT`, по умолчанию `$20,000` на ногу, и спред входа. Отдельным блоком даётся Gemini-рекомендация без фильтрации. После отчёта отдельным сообщением отправляется инструкция входа: риск депозиту, OI ниже $500k скрывается, $500k-$1m помечается ⚠️, $1m+ помечается ✅; Vol24h ниже $400k скрывается, $400k-$2m помечается ⚠️, Vol24h $2m+ помечается ✅; ордер до 1–1.5% OI, прибыльность 0.03-0.08%, спред до -0.5%, проверка объёма/стакана/arcways.io/Predicted Rate, TWAP $500-$1000, защита от пустого стакана, SL/TP сетка и верификация через Gemini.

Отдельный долгосрочный funding-скан запускается в 21:00 Europe/Kyiv, если задан `REPORT_CHAT_ID`. Он использует ту же математику, что `/longfunding`: стабильность ставок, оценка на `$20,000` на каждую ногу, минимум `$300` в месяц, затем применяет Gemini-фильтр качества актива, группирует результат одна монета = одно сообщение. OI и 24h volume не фильтруют результат, а выводятся как warning. Результаты сохраняются в памяти процесса на 30 минут и выдаются порциями по `LONGTERM_PAGE_SIZE`, по умолчанию 5 монет.
...
15. Вечерний отчёт нормализует funding по выплатам в день: 1h, 4h и 8h биржи сравниваются через дневную доходность; маршруты ниже `$10/день`, отрицательные маршруты и маршруты со spread входа ниже `-0.8%` не выводятся.
16. `/longfunding` группирует результат по монетам, показывает несколько маршрутов long/short и не фильтрует по OI/volume.
