...
Фильтр последних 6 ставок:

- применяется после основного фильтра;
- LONG остаётся актуальным, если минимум 3 из последних 6 ставок отрицательные;
- SHORT остаётся актуальным, если минимум 3 из последних 6 ставок положительные;
...
6. Фильтр последних 6 ставок отбрасывает устаревшие сигналы.
7. Gemini получает найденные монеты и возвращает фундаментально приемлемые.

## Вечерний отчёт

Запускается в 20:00 Europe/Kyiv, если задан `REPORT_CHAT_ID`.

Берёт только `full`-монеты, применяет фильтр последних 6 ставок, затем Gemini, затем ищет лучшую дельта-нейтральную пару.
