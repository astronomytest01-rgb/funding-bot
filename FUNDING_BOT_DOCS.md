...
7. OI ниже $500k скрывается в `/analyze` и вечернем отчёте; $500k-$1m показывает ⚠️, $1m+ показывает ✅.
8. `/oi` вручную проверяет OI: монета -> биржа -> рекомендация заходить или пропустить.
9. Вечерний отчёт запускается в 20:00 Europe/Kyiv при наличии `REPORT_CHAT_ID`; `/report` запускает тот же отчёт вручную.
10. Документация обновлена перед push.
