...
Берёт только `full`-монеты, применяет фильтр последних 4 ставок, затем ищет лучшую дельта-нейтральную пару, показывает оценку прибыли за 1 день и отдельным блоком даёт Gemini-рекомендацию без фильтрации. После отчёта отдельным сообщением отправляется инструкция входа: риск депозиту, прибыльность 0.03-0.08%, спред до -0.5%, проверка OI/объёма/стакана/arcways.io/Predicted Rate, TWAP $500-$1000, защита от пустого стакана, SL/TP сетка и верификация через Gemini.
